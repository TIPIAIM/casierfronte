...
import { saveDemande, getDemande } from "../../../../utils/db"; // adapte le chemin si besoin
...
function DemandeDetaildmin() {
  ...
  //
// --- NOUVELLE LOGIQUE : récupération online > sinon offline
useEffect(() => {
  let isMounted = true; // pour éviter les erreurs si composant unmounted

  async function fetchAndCache() {
    setLoading(true);
    setError(null);

    // Tente d'abord d'obtenir les données online
    if (navigator.onLine) {
      try {
        ...
        // Cache la demande dans IndexedDB
        await saveDemande({ ...data.data, id: String(id) });
      } catch (err) {
        // Si online mais fetch rate, tente le cache local
        const cached = await getDemande(String(id));
        if (cached) {
          setDemande(cached);
          setOffline(true);
          setError(null);
      ...
      // Si offline direct : lecture cache
      const cached = await getDemande(String(id));
      if (cached) {
        setDemande(cached);
        setOffline(true);
        ...
        setError(
          "Vous êtes hors-ligne et aucune donnée n'a été trouvée pour cette demande."
        );
      }
      if (isMounted) setLoading(false);
    }
  ...
  return (
    <GlobalStyles>
      ...
      {offline && (
        ...
        >
          ⚡ Affichage hors-ligne (cache local)
        </div>
      )}
      <Container>
        {/* ...toutes tes sections */}
...
export default DemandeDetaildmin;
